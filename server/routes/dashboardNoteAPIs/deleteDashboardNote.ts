...
export async function handleDeleteDashboardNote(
  ...
  if (dashboardNoteToDelete.username == userauth.username) {
        ...
        query: {
          bool: {
            must: [
              ...
              { match_phrase: { 'note.keyword': dashboardNoteToDelete.note } },
              { match_phrase: { dashboardId: dashboardNoteToDelete.dashboardId } },
            ],
          },
        },
      },
    ...
    await context.core.elasticsearch.client.asCurrentUser
      ...
      .then((res) => {
        ...
        if (res.body.total == 1 && res.body.deleted == 1) {
          retval = 'Dashboard comment deleted successfully';
          code = '200';
          return {
            body: {
              ret: retval,
          ...
          retval = 'Error: Dashboard comment was not deleted';
          code = '400';
          return {
            body: {
              ret: retval,
    ...
    retval = 'Error: Dashboard comment was not deleted-Username did not match ';
    code = '400';
    return {
      body: {
        ret: retval,
